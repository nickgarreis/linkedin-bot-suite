...
export async function sendInvitation(
  ...
  try {
    ...
    const connect = await page.$(LINKEDIN_SELECTORS.CONNECT_BUTTON);
    if (!connect) {
    ...
    if (note) {
      try {
        const noteBtn = await page.waitForSelector(LINKEDIN_SELECTORS.NOTE_BUTTON, { timeout: 5000 });
        if (noteBtn) {
          await noteBtn.click();
    ...
    const sendBtn = await page.waitForSelector(LINKEDIN_SELECTORS.SEND_BUTTON, { timeout: 5000 });
    if (!sendBtn) {
